...
int max_martingale, martingales;
...
int start()
{
   maxEB = MathMax(maxEB, AccountEquity()/AccountBalance());
   minEB = MathMin(minEB, AccountEquity()/AccountBalance());
...
int Sinal() {
   ...
   if (Bid < Low[1] + (Low[1] + High[1]) * 0.4)
      return (OrderSend(Symbol(), OP_BUY, contra_pips(contrapips), Ask, 1, 0, 0));
...
double contra_pips(double pips) {
   double l = AccountEquity();
   double d = (1 - ((Ask-pips)/Ask + 1/AccountLeverage())) * 100000;
   Alert(d, l/d);
   return (NormalizeDouble(l/d, 2));
}
...
void tela() {
   ...
   Comment(
      ...
      StringConcatenate("\n", "martingales: ", martingales, "  max: ", max_martingale, "\n"),
      
      StringConcatenate("\nbal: ", AccountBalance(), " ", AccountCurrency()),
      StringConcatenate("\nequity: ", AccountEquity(), " ", AccountCurrency()),
      StringConcatenate("\nequity/bal: ", AccountEquity()/AccountBalance(), " (min=", minEB, "[",margem,"],max=", maxEB, ")\n"),
