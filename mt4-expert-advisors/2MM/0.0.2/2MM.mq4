...
extern string   str3                  = "Filtro de Horário";
...
int deinit()
{
  ...
  if (indicar_sucesso_posicoes) {
    for (i=0; i<hist_index_i; i++) {
      if (OrderSelect(hist_index[i], SELECT_BY_POS, MODE_HISTORY) && OrderMagicNumber() == MAGIC_NUM) {
        ObjectCreate("2MM-pos-" + i, OBJ_ARROW, 0, OrderOpenTime(),
          Low[iBarShift(Symbol(), 0, OrderOpenTime())] - PipsParaPreco(80));
        if (OrderProfit() >= 0) {
          ObjectSet("2MM-pos-" + i, OBJPROP_ARROWCODE, SYMBOL_CHECKSIGN);
          ObjectSet("2MM-pos-" + i, OBJPROP_COLOR, Green);
        } else if (OrderProfit() < 0) {
          ObjectSet("2MM-pos-" + i, OBJPROP_ARROWCODE, SYMBOL_STOPSIGN);
          ObjectSet("2MM-pos-" + i, OBJPROP_COLOR, Red);
        }
      }
    }
  }
  ...
  if (gerar_vetores_caracteristicas) {
    string nome_arquivo = "2MM-testing.dat";
...
int start()
{
    ...
    (Aplicar_Filtro_de_Horario && Filtro_Horario(Horario_Inicio, Horario_Fim)))
  {
    ...
    {
      // tendência de alta: Cruzamento das médias móveis rápida e média
      if (rapida_1 < lenta_1 && rapida_0 > lenta_0) {
      ...
      // tendência de baixa: Cruzamento das médias móveis rápida e média
      /*if (rapida_1 > lenta_1 && rapida_0 < lenta_0) {
...
bool Desenhar(int ticket, int q)
{
  if (OrderSelect(ticket, SELECT_BY_TICKET, MODE_TRADES)) {
    ...
    case ABERTURA:
      switch (OrderType()) {
      case OP_BUY:
        if (!ObjectCreate("2MM-sl-" + ticket, OBJ_HLINE, 0, 0, preco_abertura -
                          PipsParaPreco(SL_pips)))
          return (false);
        
        if (!ObjectCreate("2MM-tp1-" + ticket, OBJ_HLINE, 0, 0, preco_abertura +
                          PipsParaPreco(TP_N1_pips))) {
          ObjectDelete("2MM-tp1-" + ticket);
          return (false);
        }
        
        ObjectSet("2MM-sl-" + ticket, OBJPROP_COLOR, Red);
        ObjectSet("2MM-sl-" + ticket, OBJPROP_WIDTH, 2);
        ObjectSet("2MM-tp1-" + ticket, OBJPROP_COLOR, DarkGreen);
    ...
    case TPN1_ATINGIDO:
      ...
      case OP_BUY:
        ...
        if (!ObjectCreate("2MM-tp2-" + ticket, OBJ_HLINE, 0, 0, preco_abertura +
                          PipsParaPreco(TP_N2_pips)))
        {
          ObjectDelete("2MM-tp2-" + ticket);
          return (false);
        }
        
        ObjectSet("2MM-sl-" + ticket, OBJPROP_COLOR, Red);
        ObjectSet("2MM-sl-" + ticket, OBJPROP_WIDTH, 1);
        ObjectSet("2MM-tp2-" + ticket, OBJPROP_COLOR, Green);
    ...
    case TPN2_ATINGIDO:
      ...
      case OP_BUY:
        ...
        if (!ObjectCreate("2MM-sl-" + ticket, OBJ_HLINE, 0, 0, sl))
          return (false);
        
        if (!ObjectCreate("2MM-tp3-" + ticket, OBJ_HLINE, 0, 0, preco_abertura +
                          PipsParaPreco(TP_N3_pips)))
        {
          ObjectDelete("2MM-tp3-" + ticket);
          return (false);
        }
        
        ObjectSet("2MM-sl-" + ticket, OBJPROP_COLOR, Maroon);
        ObjectSet("2MM-sl-" + ticket, OBJPROP_WIDTH, 1);
        ObjectSet("2MM-tp3-" + ticket, OBJPROP_COLOR, Lime);
...
{
  if (ObjectFind("2MM-sl-"  + ticket) != -1)
    if (!ObjectDelete("2MM-sl-"  + ticket))
      return (false);
  
  if (ObjectFind("2MM-tp1-" + ticket) != -1)
    if (!ObjectDelete("2MM-tp1-" + ticket))
      return (false);
  
  if (ObjectFind("2MM-tp2-" + ticket) != -1)
    if (!ObjectDelete("2MM-tp2-" + ticket))
      return (false);
  
  if (ObjectFind("2MM-tp3-" + ticket) != -1)
    if (!ObjectDelete("2MM-tp3-" + ticket))
      return (false);
