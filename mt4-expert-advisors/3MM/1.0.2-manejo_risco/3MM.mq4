...
bool AbrePosicao(int ordem, double tamanho_posicao)
{
  ...
  // bal_max <- max(bal_max, acc_balance) | min(bal_max, acc_balance) | acc_bal
  balanco_maximo = MathMax(balanco_maximo, AccountBalance());
  //balanco_maximo = MathMin(balanco_maximo, AccountBalance());
  double tam = (balanco_maximo * tamanho_posicao)/10000;
  
  int ticket;
  if (ordem == OP_BUY) {
    ticket = OrderSend(Symbol(), OP_BUY , tam, Ask, 0, Ask - PipsParaPreco(SL_pips), 0,
      "", MAGIC_NUM);
    if (ticket != -1) {
      // Desenha nível de SL
      if (Desenhar_TP_e_SL)
    ...
    ticket = OrderSend(Symbol(), OP_SELL, tam, Bid, 0, Bid + PipsParaPreco(SL_pips), 0,
      "", MAGIC_NUM);
    if (ticket != -1) {
