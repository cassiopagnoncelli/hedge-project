...
extern string   str5                  = "Posições permitidas (1:long,2:short)";
extern int      PosicoesPermitidas    = 3;
extern bool     DataMining            = false;
...
double caracteristicas[5][1024];   // primeiro eixo dos vetores
...
int init()
{
  ...
  gerar_vetores_caracteristicas = DataMining;
...
int deinit()
{
  ...
  if (gerar_vetores_caracteristicas) {
    ...
    if (fp != -1) {
      FileWrite(fp, "alpha_L alpha_R alpha Psar");
    ...
    fp = FileOpen(nome_arquivo, FILE_CSV | FILE_WRITE, ' ');
    if (fp != -1) {
      FileWrite(fp, "alpha_L alpha_R alpha Psar teve_lucro");
      for (i=0; i<caracteristicas_i; i++)
        if (OrderSelect(hist_index[i], SELECT_BY_POS, MODE_HISTORY) && OrderMagicNumber() == MAGIC_NUM)
          FileWrite(fp,
            OrderType()==OP_BUY,
    ...
    fp = FileOpen(nome_arquivo, FILE_CSV | FILE_WRITE, ' ');
    if (fp != -1) {
      FileWrite(fp, "alpha_L alpha_R alpha Psar lucro");
      for (i=0; i<caracteristicas_i; i++)
        if (OrderSelect(hist_index[i], SELECT_BY_POS, MODE_HISTORY) && OrderMagicNumber() == MAGIC_NUM)
          FileWrite(fp,
            OrderType()==OP_BUY,
...
int start()
{
  ...
  double
    ...
    mm        = iMA(Symbol(), 0, 5, 0, MODE_SMA, PRICE_CLOSE, Aguardar_Formacao_Candle==true);
    ...
    (Aplicar_Filtro_de_Horario && Filtro_Horario(Horario_Inicio, Horario_Fim)))
  {
    ...
    {
      // tendência de alta
      if (PosicoesPermitidas == 1 || PosicoesPermitidas == 3)
        if (mm > env_up) {
      ...
      // tendência de baixa
      if (PosicoesPermitidas == 2 || PosicoesPermitidas == 3)
        if (mm < env_down) {
...
bool AbrePosicao(int ordem, double tamanho_posicao)
{
  ...
  if (gerar_vetores_caracteristicas) {
    caracteristicas[0][caracteristicas_i] = iCustom(Symbol(), Period(), "MM_inclinacao", 
      Reta_Regressao_Lenta,  Periodo_MM_Lenta,  false, 1, DarkOliveGreen, 3, 0,
      0, 0);
    caracteristicas[1][caracteristicas_i] = iCustom(Symbol(), Period(), "MM_inclinacao", 
      Reta_Regressao_Rapida, Periodo_MM_Rapida, false, 0, DarkKhaki,      1, 0,
      0, 0);
    caracteristicas[2][caracteristicas_i] = MathArctan(caracteristicas[1][caracteristicas_i]
      / caracteristicas[0][caracteristicas_i]);
    caracteristicas[3][caracteristicas_i] = MathAbs(iSAR(Symbol(), 0, 0.002, 0.01, 0) - Ask);
    caracteristicas_i++;
  }
