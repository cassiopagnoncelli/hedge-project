...
extern double DrawdownMaximal = 0.5;
extern double step=75; //original:25
...
extern double proffactor=6;//10
extern double mult=1.3;//1.5
extern double lotsbuy=0.01;
extern double lotssell=0.01;  
extern double per_K=60;//200
extern double per_D=12;//20
extern double slow=12;//20
extern double zoneBUY=290;
extern double zoneSELL=290;
...
int cnt;   
                                                 ...
                                                 int start()
  {
   ...
   // drawdown maximal
   if ( AccountEquity()/AccountBalance() < DrawdownMaximal ) {
      int i;
      for ( i=0; i<OrdersTotal(); i++ ) 
         if ( OrderSelect(i, SELECT_BY_POS, MODE_TRADES) && OrderCloseTime() == 0 && (OrderMagicNumber() == Magicbuy || OrderMagicNumber() == Magicsell) )
            switch (OrderType()) {
               case OP_BUY:
